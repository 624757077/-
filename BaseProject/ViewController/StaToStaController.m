...
@interface StaToStaController ()<UIPickerViewDataSource,UIPickerViewDelegate,UITextFieldDelegate>
...
@implementation StaToStaController
...
- (IBAction)tiaoDaoXiangQingYe:(id)sender {
    ...
    if (self.startStation.text.length == 0 || self.endStation.text.length == 0) {
    ...
    StaToStaDetailViewController *vc = [[StaToStaDetailViewController alloc]init];
    self.staTostaVM = [StationToStationViewModel new];
    self.staTostaVM.startStation = self.startStation.text;
    self.staTostaVM.endStation = self.endStation.text;
    self.staTostaVM.date = self.showStr;
    self.staTostaVM.type = self.typeStr;
    
    vc.staToStaVM = self.staTostaVM;
